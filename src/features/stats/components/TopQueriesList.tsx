...
const MOCK: TopQuery[] = [
  ...
  { query: '백로그 항목을 어떻게 관리하나요?', count: 38 },
  { query: 'Jira 연동 에러가 발생합니다', count: 29 },
  { query: '새 프로젝트 생성 권한', count: 21 },
  { query: '팀원 초대 링크가 만료되었습니다', count: 15 },
];

export default function TopQueriesList({ data, loading }: Props) {
  const list = data.length > 0 ? data : MOCK;
  const max = Math.max(...list.map((q) => q.count));
    ...
    <div className="bg-white dark:bg-[#1e2235] rounded-2xl p-6 shadow-sm border border-gray-100 dark:border-[#2a2f45]">
      ...
      {loading ? (
        <div className="space-y-4">
          {[...Array(5)].map((_, i) => (
            <div key={i} className="h-8 bg-gray-100 dark:bg-[#2a2f45] rounded animate-pulse" />
          ))}
        </div>
      ) : (
        <div className="space-y-4">
            ...
            <div key={idx} className="flex items-center gap-3">
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center justify-between mb-1">
                  <span className="text-sm text-gray-700 dark:text-gray-200 truncate pr-2">{q.query}</span>
                  <span className="text-xs text-gray-400 shrink-0">{q.count}건</span>
                </div>
